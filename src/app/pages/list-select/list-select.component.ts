...
export class ListSelectComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    if (this.list === 'alphabet') {
      if (
        !this.progressService.getReceivedInstructions(
          'hasReceivedAlphabetInstruction'
        )
      ) {
        this.playAudio();
        this.progressService.setReceivedInstructions(
          'hasReceivedAlphabetInstruction',
          true
        );
      }
    ...
    if (this.activatedRoute.snapshot.queryParamMap.get('list') == 'alphabet') {
      ...
      for (
        var i = 0;
        i < Object.keys(document.getElementsByClassName('bottom')).length - 1;
        i++
      ) {
        document
          .getElementsByClassName('bottom')
          [i].classList.add('alphabet-list-bottom');
      }
    }
  }
  ...
  ngAfterViewInit() {
    if (this.list === 'alphabet') {
      for (
        var i = 0;
        i < document.getElementsByClassName('app-card').length;
        i++
      ) {
  ...
  showProgress(item): void {
    let numGoldStars;
    let numSilverStars;
    let elem = document.getElementsByClassName('cardListItem')[
      this.cardItemCount
    ];
    let queryStatement;
    ...
    if (item != null) {
      if (!this.dataProgress.includes(item)) {
        ...
        if (this.list === 'alphabet') {
          ...
          numSilverStars = this.progressService.getSilverStarsFromKey(
            queryStatement
          );
        } else {
        ...
        for (let i = 0; i < numGoldStars; i++) {
          ...
          if (this.list === 'alphabet') {
            ...
            if (
              Object.keys(document.getElementsByTagName('app-card')).length > 6
            ) {
